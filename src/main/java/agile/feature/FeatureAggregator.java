...
public class FeatureAggregator<T> implements AgileObject {
    ...
    /*
     * This checks if the csl's feature set has a program feature
     */
    public boolean doesThisCSLHaveThisProg(String key, Feature feature)
    {
        boolean bool = false;
        if (featureMap.containsKey(key))
        {
            if(featureMap.get(key).containsFeature(feature))
            {
                bool = true;
            }
        }
